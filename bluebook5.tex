...
\begin{itemize}
\item A node, $n = \langle t, l\rangle$, is a proof tree for $F$ and $R$ if $t \in F$, $l = initial$ and it has no outgoing edges connecting it to other nodes.
\item A node, $n = \langle t, l\rangle$, is a proof tree for $F$ and $R$ if $l = unprovable$ and no proof tree for $F$ and $R$, $pt(F, R)$, exists such that $t \in pt(F, R)$
\item Let ${\cal T}$ be a set of proof trees for $F$ and $R$.  

If there exists a rule $\drule{l}{A}{C}$ and a substitution, $\theta$ for the free variables in $C$ such that $C\theta = t$ and for each positive literal $t' \in A$ there exists a $T \in {\cal T}$ such that $t' \in T$ and for each negative literal $\neg t' \in A$ there exists a $T \in {\cal T}$ such that $n' = \langle t'\theta, unprovable \rangle \in T$ then 

a node, $n' = \langle t'\theta, l'\rangle$ which is connected to $n$ by an outgoing edge from $n$ to $n'$ (each of the antecedants of the rule is matched by a parent node for $n$) and for every node, $n' = \langle t', l'\rangle$ connected to $n$ by an outgoing edge from $n$ there exists a term $t'' \in A$ such that $t''\theta = t'$.  For each negative literal $\neg t' \in A$ there is no node $n' = \langle t'\theta, l'\rangle$ in the proof graph.
