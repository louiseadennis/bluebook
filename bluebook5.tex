...
A \emph{proof tree}, $\pt{F}{R}$, for a set of facts, $F$, and rules, $R$ is defined recursively as follows:

\begin{itemize}
\item A node, $n = \langle t, initil\rangle$, is a proof tree for $F$ and $R$ if $t \in F$ and it has no outgoing edges connecting it to other nodes.
\item A node, $n = \langle t, unprovable\rangle$, is a proof tree for $F$ and $R$ if no proof tree for $F$ and $R$, $pt(F, R)$, exists such that $t \in pt(F, R)$
\item Let ${\cal T} = \{T_1, \ldots, T_n\}$ be a set of proof trees for $F$ and $R$.  

  If there exists a rule $\drule{l}{A}{C}$ and a substitution, $\theta$ for the free variables in $C$ such that $C\theta = t$ and for each positive literal $t' \in A$ there exists a $T_i \in {\cal T}$ such that $t' \in T_i$ and for each negative literal $\neg t' \in A$ there exists a $T_i \in {\cal T}$ such that $\neg t' \in T$ then $\langle \bigcup_i N_i \cup \{\langle C\theta, l \rangle\}, \bigcup_i E \cup E'\rangle$ where $E' = \{\langle \langle C\theta, l \rangle, \langle t', l' \rangle \rangle \mid (t \in pos(A) \wedge \exists i. \: t \in T_i) \vee (t \in neg(A) \wedge \exists i. \: \neg t \in T_i) \}$ is a proof tree for $F$ and $R$.
...
\begin{enumerate}
\item If $stmt_{k + 1} = initial(t)$ then $stmt_{k} = why(t)$ and $t \in F_{i_k}$.
\item If $stmt_{k + 1} = \drule{l}{A}{t}$ then $stmt_{k} = why(t)$, $t \not\in F_{i_k}$ and for some node $n = \langle t, l\rangle \in G_{i_k}$ and $A$ are the terms for $n$'s parent nodes.
\item If $stmt_{k + 1} = why(t)$ then $stmt_{k} =  whynot(t)$.
\item If $stmt_{k + 1} = whynot(t)$ then $stmt_{k} \neq why(t)$, $\forall l. l \leq k \rightarrow stmt_{l} \neq whynot(t)$ (this question has not been asked before) and $t \in G_{i_k} \cap N_{{i_k}j}$ (Player ${i_k}$ identifies a term $t$ that it believes and it has established the other doesn't  and asks why not).
\item If $stmt_{k + 1} = why(t)$ then $\forall l. l \leq k \rightarrow stmt_{l} \neq why(t)$ (this question has not been asked before), $stmt_{k} \neq whynot(t)$ and $t \in Y_{{i_k}j}\backslash G_{i_k}$ (Player ${i_k}$ identifies a term $t$ that it does not believe and it has established the other does  and asks why).
\item If $stmt_{k + 1} = \mathit{different\_fact}(t, j, {i_k})$ then $t \in F_{{i_k}j}$ and $t \not\in F_{i_k}$ (Player ${i_k}$ has identified that $t$ was an initial fact for Player $j$ but not for Player ${i_k}$)
\item If $stmt_{k + 1} = \mathit{different\_fact}(t, {i_k}, j)$ then $t \in N_{{i_k}j}$ and $t \in F_{i_k}$ (Player ${i_k}$ has identified that $t$ was an initial fact for Player ${i_k}$ but not for Player $j$)
\item If $stmt_{k+1} = \mathit{different\_rule}(l, j, {i_k})$ then $l \in YR_{{i_k}j}$ and $l \not\in R_{i_k}$ (Player ${i_k}$ has identified that $l$ is a rule for  Player $j$ but not for Player ${i_k}$)
\end{enumerate}
...
\begin{theorem}
If the $k$th state in a legal sequence of dialogue states is $\langle S^1_{k}, S^2_{k}, s, i \rangle$ and $s \neq  \mathit{different\_fact}(t, i, j),  \mathit{different\_fact}(t, j, i)$ nor $\mathit{different\_rule}(l, i, j)$ then there is a legal next dialogue state.
\end{theorem}
\begin{proof}

First we establish that in all dialogue states one of the following must hold:
\begin{enumerate}
\item $G_i \cup N_{ij} \neq \emptyset$
...
\begin{enumerate}
\item {\bf dialogue start.} In this case $Y_{12} = \{t\}$ and $N_{21} = \{t\}$ for some $t \in G_2$ and $t \not\in G_1$ (or vice versa).  So either 1 or 2 above is true.
...
We note that if $s = why(t)$ or $s = whynot(t)$ then a legal next move is always possible.  This follows from the corrollaries to theorems~\ref{thm:Ycorrect} and \ref{thm:Ncorrect}.
